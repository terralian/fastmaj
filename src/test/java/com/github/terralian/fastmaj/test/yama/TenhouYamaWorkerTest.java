...
public class TenhouYamaWorkerTest {
    ...
    public void officalTest() throws NoSuchAlgorithmException {
        ...
        IYamaArray yama = parser.getNextYama(1);
        // 官方提供的验证结果
        String predicalYama =
        ...
        assertEquals(predicalYamaArray.length, yama.value().length);
        for (int i = 0; i < yama.value().length; i++) {
            assertEquals(Integer.valueOf(predicalYamaArray[i]).intValue(), yama.value()[i]);
        }
    }
