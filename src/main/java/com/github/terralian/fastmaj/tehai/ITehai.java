 ...
 * <p/>
 * 在设计上{@link ITehai}基于对象（即{@link IHai}）进行类间交互，这种设计易于扩展，但是不易使用。
 * 对此增加了手牌构建类{@link TehaiBuilder}，支持手牌的快速构建。
 *
 * @author terra.lian
 * @see Encode34
 * @see EncodeMark
...
public interface ITehai {
     ...
     * <p/>
     * 当手牌未进行鸣牌时，该方法获取的值与{@link ITehai#getHand()}一致。获取到的牌集合的排序为入手顺序，所以是乱序排列。
     ...
     * 暗杠后，该方法仅获取其中未进行鸣牌暗杠的部分。若想获取到副露的部分，可以使用{@link #getLock()}方法。
     * <p/>
     * 获取到的牌集合的排序为入手顺序，所以是乱序排列。要排序的场合可以使用{@link #sort(List)}手动对手牌进行排序。
     * 对于需要修改牌形状，如打出某枚牌，鸣牌等，直接修改该返回集合不会对内部值产生影响，请使用对应的操作方法。
     * <p/>
     ...
     * 获取手牌固定区，或者称其为副露区
     * <p/>
     * 固定区为鸣牌或者暗杠后将牌固定的桌右角，可通过该区域查询固定信息。副露区对象将在{@link ITehai}初始化时创建，
     * 一副手牌是否鸣牌可以通过{@link #isNaki()}查询，或者{@link ITehaiLock#isNaki()}进行判断。
     *
     * @return ITehaiLock
     */
    ITehaiLock getLock();
    ...
    /**
     * 把摸到的最后一枚牌进行弃牌，需要手牌为3n+2的状态（即手牌刚摸进一枚牌）
     *
     * @return 默认为false，模切
     * @throws IllegalStateException 若非3n+2时，抛出该异常
     */
    IHai kiriLast() throws IllegalStateException;

    /**
     * 吃动作，通过指定搭子牌，将上家最后一枚弃牌加入到自己的手牌副露区，完成吃牌的操作。
     * <p/>
     * 该操作内部会首先调用{@link #draw(IHai)}将牌加入手中
     *
     * @param chiHai 吃的牌
     * @param tehai1 搭子牌1
     * @param tehai2 搭子牌2
    ...
    /**
     * 碰动作，通过制定搭子牌，将对手的最后一枚棋牌加入到自己的手牌副露区，完成碰牌操作。
     * <p/>
     * 该操作会首先调用{@link #draw(IHai)}将牌加入手中。
     * <p/>
     * 设计上，若手牌中存在超过可碰牌的枚数时，对游戏本身而言除非存在红宝牌，否则使用哪两枚作为搭子都没有差异。
     * 于是该方法将仅考虑是否优先使用宝牌，而非指定使用哪两张牌作为搭子。
     *
     * @param ponHai 碰的牌
     ...
     * @param ponHai 碰的牌
     * @param fromPlayer 来源玩家
     * @see #pon(IHai, boolean, RivalEnum)
     */
