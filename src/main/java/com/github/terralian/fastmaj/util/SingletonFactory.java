...
public abstract class SingletonFactory {
     ...
     * 
     * @param <T> 接口类型
     * @param intefaceClass 接口类
     * @param actualClass 实例类
     * @throws IllegalArgumentException 若实例类不能简单创建，会报该异常
     */
    @SuppressWarnings("unchecked")
    public static <T> T doGetBean(Class<T> intefaceClass, Class<? extends T> actualClass) {
        String className = intefaceClass.getName();
    ...
    /**
     * 仅获取接口对应的实例，配合{@link #registeBean}来缓存复杂构建的实例
     * 
     * @param <T>
     * @param intefaceClass
     */
    @SuppressWarnings("unchecked")
    public static <T> T onlyGet(Class<T> intefaceClass) {
        String className = intefaceClass.getName();
        Object obj = singletonMap.get(className);
     ...
     * @param obj 实例
     */
    public static <T> void registeBean(Class<T> intefaceClass, Object obj) {
        synchronized (SingletonFactory.class) {
            String className = intefaceClass.getName();
            singletonMap.put(className, obj);
        }
