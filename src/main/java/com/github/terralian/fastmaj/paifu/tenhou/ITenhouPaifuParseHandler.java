 ...
 * 天凤牌谱的解析处理器接口
 * <p/>
 * 在{@link TenhouPaifuDecodeHandler}对牌谱解码的过程中，在遇到对应牌谱的事件节点时，
 * 会调用该接口的对应方法，处理实际的解析业务。
 * <p/>
 * 一个使用的方法为,根据需求实现该接口的实例，并通过创建{@link TenhouPaifuParser}，
 * 将该类作为构造参数传入。通过{@link TenhouPaifuParser#parseContent}等方法进行解析，最后取得所需的返回值。
 * 示例代码如下：
 * <pre>
 * // 实现一个所需业务的解析处理器接口
 * class MyTenhouPaifuParseHandler extends MyTenhouPaifuParseHandler {};
 * // 构造 TenhouPaifuParser 或者其他解析器接口
 * IPaifuParser paifuParser = new TenhouPaifuParser(new MyTenhouPaifuParseHandler());
 * // 进行解析
 * paifuParser.parseContent("fileUrl");
 * // 获取返回值
 *
 * </pre>
 *
